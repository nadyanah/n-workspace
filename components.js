...
const CalendarMoment = {
  template: `
    <div class="calendar-moment animate-fade-in" style="width: 100%; padding: 0;">
        ...
        <div style="background: #FAF6F0; border: 1.5px solid var(--color-sand); border-radius: 14px; padding: 12px 16px; display: flex; flex-wrap: wrap; gap: 8px; align-items: center;">
          ...
          <div style="display: flex; gap: 2px; background: #fff; border: 1.5px solid var(--color-sand); border-radius: 8px; padding: 2px; flex-shrink: 0;">
            ...
            <button type="button" @click="currentView = 'agenda'"
                    :style="currentView==='agenda' ? {background:'var(--color-terracotta)',color:'#fff'} : {background:'transparent',color:'#5D4F43'}"
                    style="border:none; font-size:11.5px; padding:0 10px; border-radius:6px; font-weight:700; height:30px; display:inline-flex; align-items:center; gap:4px; cursor:pointer; transition:all 0.15s; white-space:nowrap;">
              <svg viewBox="0 0 24 24" width="11" height="11" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="M8 6h13M8 12h13M8 18h13M3 6h.01M3 12h.01M3 18h.01"/></svg>
              Agenda
            </button>
          ...
          <!-- Navigasi bulan (calendar & agenda view) -->
          <div v-if="currentView === 'calendar' || currentView === 'agenda'" style="display:inline-flex; align-items:center; gap:2px; background:#fff; border:1.5px solid var(--color-sand); border-radius:8px; padding:2px 4px; flex-shrink:0;">
      ...
      <!-- VIEW B: AGENDA — Semua event dari semua modul per tanggal -->
      <div v-else-if="currentView === 'agenda'" class="animate-fade-in" style="width: 100%;">

        <!-- Stat chips ringkasan bulan -->
        <div style="display:flex; gap:10px; margin-bottom:18px; flex-wrap:wrap;">
          <div style="background:#FAF0EC; border:1px solid #F3D8CC; border-radius:10px; padding:8px 14px; text-align:center;">
            <div style="font-size:16px; font-weight:800; color:var(--color-terracotta); line-height:1;">{{ agendaSummary.totalItems }}</div>
            <div style="font-size:10px; color:var(--text-muted); font-weight:600;">total agenda</div>
          </div>
          <div style="background:#F0F9F0; border:1px solid #C6E6C8; border-radius:10px; padding:8px 14px; text-align:center;">
            <div style="font-size:16px; font-weight:800; color:#16a34a; line-height:1;">{{ agendaSummary.totalDays }}</div>
            <div style="font-size:10px; color:var(--text-muted); font-weight:600;">hari aktif</div>
          </div>
          <div style="background:#EFF6FF; border:1px solid #BFDBFE; border-radius:10px; padding:8px 14px; text-align:center;">
            <div style="font-size:16px; font-weight:800; color:#2563eb; line-height:1;">{{ agendaSummary.todayCount }}</div>
            <div style="font-size:10px; color:var(--text-muted); font-weight:600;">agenda hari ini</div>
          </div>
          <!-- Tambah Pengingat button -->
          <button type="button" @click="showAgendaForm = !showAgendaForm"
                  style="margin-left:auto; height:auto; padding:8px 16px; background:var(--color-terracotta); color:#fff; border:none; border-radius:10px; font-size:12px; font-weight:700; cursor:pointer; display:inline-flex; align-items:center; gap:6px; box-shadow:0 2px 8px rgba(214,123,82,0.2);">
            <svg viewBox="0 0 24 24" width="13" height="13" fill="none" stroke="currentColor" stroke-width="2.5"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="16"/><line x1="8" y1="12" x2="16" y2="12"/></svg>
            {{ showAgendaForm ? 'Tutup Form' : '+ Pengingat Baru' }}
          </button>
        </div>

        <!-- Form tambah pengingat langsung dari Agenda -->
        <transition name="notif-manual-expand">
          <div v-if="showAgendaForm"
               style="background:#FFFFFF; border:1.5px solid var(--color-sand); border-radius:14px; padding:18px; margin-bottom:20px; box-shadow:0 2px 12px rgba(0,0,0,0.05);">
            <div style="font-size:13px; font-weight:800; color:#3E352F; margin-bottom:14px; display:flex; align-items:center; gap:7px;">
              <span style="font-size:15px;">⏰</span> Tambah Pengingat ke Kalender
            </div>
            <div style="display:grid; grid-template-columns:1fr 1fr; gap:10px; margin-bottom:10px;">
              <div style="grid-column:1/-1;">
                ...
                <input v-model="agendaForm.title" type="text" placeholder="cth., Meeting klien, Minum obat..." maxlength="60"
                       style="width:100%; padding:8px 11px; border:1.5px solid var(--color-sand); border-radius:8px; font-size:12.5px; color:var(--text-dark); background:#fff; outline:none; box-sizing:border-box;" />
              </div>
              <div>
                <label style="font-size:11px; font-weight:600; color:var(--text-muted); display:block; margin-bottom:4px;">Tanggal *</label>
                <input v-model="agendaForm.date" type="date"
                       style="width:100%; padding:8px 11px; border:1.5px solid var(--color-sand); border-radius:8px; font-size:12px; color:var(--text-dark); background:#fff; outline:none; box-sizing:border-box;" />
              </div>
              <div>
                <label style="font-size:11px; font-weight:600; color:var(--text-muted); display:block; margin-bottom:4px;">Jam *</label>
                <input v-model="agendaForm.time" type="time"
                       style="width:100%; padding:8px 11px; border:1.5px solid var(--color-sand); border-radius:8px; font-size:12px; color:var(--text-dark); background:#fff; outline:none; box-sizing:border-box;" />
              </div>
              <div style="grid-column:1/-1;">
                ...
                <input v-model="agendaForm.subtitle" type="text" placeholder="Catatan singkat..." maxlength="80"
                       style="width:100%; padding:8px 11px; border:1.5px solid var(--color-sand); border-radius:8px; font-size:12px; color:var(--text-dark); background:#fff; outline:none; box-sizing:border-box;" />
              </div>
            </div>
            <div style="display:flex; gap:8px;">
              <button type="button" @click="showAgendaForm = false; agendaForm = { title:'', subtitle:'', time:'', date:'' }"
                      style="flex:1; padding:8px; background:#FAF6F0; border:1.5px solid var(--color-sand); border-radius:8px; font-size:12px; font-weight:600; color:var(--text-dark); cursor:pointer;">
                Batal
              </button>
              <button type="button" @click="saveAgendaReminder"
                      :disabled="!agendaForm.title.trim() || !agendaForm.date || !agendaForm.time"
                      style="flex:2; padding:8px; background:var(--color-terracotta); border:none; border-radius:8px; font-size:12px; font-weight:700; color:#fff; cursor:pointer; transition:opacity 0.15s;"
                      :style="{ opacity: (!agendaForm.title.trim() || !agendaForm.date || !agendaForm.time) ? 0.5 : 1 }">
                Simpan ke Kalender
              </button>
            </div>
          </div>
        ...
        <!-- Empty state -->
        <div v-if="agendaGrouped.length === 0"
             style="padding:60px 24px; text-align:center; background:#FFFFFF; border:1.5px dashed var(--color-sand); border-radius:18px;">
          <div style="font-size:28px; margin-bottom:10px;">📭</div>
          <div style="font-size:13.5px; font-weight:700; color:#8F8175; margin-bottom:4px;">Tidak ada agenda di {{ currentMonthName }}</div>
          <div style="font-size:12px; color:var(--text-muted);">Tambahkan pengingat, task di Job Logbook, atau Habit Tracker</div>
        </div>

        <!-- Agenda list per tanggal -->
        <div v-else style="display:flex; flex-direction:column; gap:14px;">
          <div v-for="group in agendaGrouped" :key="group.dateStr"
               style="background:#FFFFFF; border:1.5px solid var(--color-sand); border-radius:14px; overflow:hidden; box-shadow:0 2px 8px rgba(0,0,0,0.04);">

            <!-- Date header -->
            <div :style="group.isToday ? 'background:linear-gradient(135deg,#FAF0EC,#FFF5F0)' : 'background:#FAF8F5'"
                 style="padding:10px 16px; display:flex; align-items:center; gap:10px; border-bottom:1px solid #FAF0E8;">
              <div :style="group.isToday ? 'background:var(--color-terracotta)' : 'background:#EDE8E2'"
                   style="width:36px; height:36px; border-radius:10px; display:flex; flex-direction:column; align-items:center; justify-content:center; flex-shrink:0;">
                <span :style="group.isToday ? 'color:#fff' : 'color:#5D4F43'" style="font-size:14px; font-weight:800; line-height:1;">{{ group.dayNum }}</span>
                <span :style="group.isToday ? 'color:rgba(255,255,255,0.8)' : 'color:var(--text-muted)'" style="font-size:8.5px; font-weight:700; text-transform:uppercase;">{{ group.dayName }}</span>
              </div>
              <div style="flex:1; min-width:0;">
                <div style="font-size:13px; font-weight:800; color:#3E352F; line-height:1.2;">
                  {{ group.fullDate }}
                  <span v-if="group.isToday" style="margin-left:6px; font-size:10px; background:var(--color-terracotta); color:#fff; padding:1px 7px; border-radius:8px; font-weight:700; vertical-align:middle;">Hari Ini</span>
                </div>
                <div style="font-size:10.5px; color:var(--text-muted); margin-top:1px;">{{ group.items.length }} agenda</div>
              </div>
            </div>
            ...
            <div style="padding:10px 14px; display:flex; flex-direction:column; gap:7px;">
              <div v-for="item in group.items" :key="item.id"
                   style="display:flex; align-items:center; gap:10px; padding:8px 12px; border-radius:10px; border:1px solid;"
                   :style="{ background: item.color + '0D', borderColor: item.color + '30' }">
                <!-- Source icon -->
                <div :style="{ background: item.color + '20' }"
                     style="width:28px; height:28px; border-radius:8px; display:flex; align-items:center; justify-content:center; flex-shrink:0; font-size:13px;">
                  {{ item.icon }}
                </div>
                <!-- Content -->
                <div style="flex:1; min-width:0;">
                  <div style="font-size:12.5px; font-weight:700; color:#3E352F; overflow:hidden; text-overflow:ellipsis; white-space:nowrap;">{{ item.title }}</div>
                  <div style="font-size:10.5px; color:var(--text-muted); display:flex; align-items:center; gap:6px; margin-top:1px;">
                    <span style="font-weight:600;">{{ item.source }}</span>
                    <span v-if="item.subtitle && item.subtitle !== item.source" style="overflow:hidden; text-overflow:ellipsis; white-space:nowrap;">· {{ item.subtitle }}</span>
                  </div>
                </div>
                <!-- Time badge -->
                <div v-if="item.time" :style="{ background: item.color + '15', color: item.color }"
                     style="flex-shrink:0; font-size:11px; font-weight:800; padding:3px 9px; border-radius:20px; border:1px solid; border-color:inherit;">
                  {{ item.time }}
                </div>
                <!-- Delete button (hanya untuk pengingat manual) -->
                <button v-if="item.isManual" type="button" @click="deleteAgendaReminder(item.id)"
                        style="flex-shrink:0; background:none; border:none; cursor:pointer; color:#DC2626; font-size:15px; padding:2px 5px; border-radius:6px; line-height:1; opacity:0.6; transition:opacity 0.15s;"
                        onmouseover="this.style.opacity='1'" onmouseout="this.style.opacity='0.6'" title="Hapus">
                  ×
                </button>
              </div>
            </div>
...
const GoogleCalendar = {
    ...
    <div class="google-calendar-screen">
      ...
      <div v-if="needsAuth">
        ...
        <div v-if="localView==='agenda'" class="gcal-agenda-wrap">
          ...
          <!-- ══ PANEL RINGKASAN SEMUA MODUL ══ -->
          <div style="margin-bottom:20px; background:#FFFFFF; border:1.5px solid var(--color-sand); border-radius:16px; overflow:hidden; box-shadow:0 2px 12px rgba(0,0,0,0.05);">

            <!-- Header panel -->
            <div style="padding:12px 16px 10px; background:linear-gradient(135deg,#FDF8F4,#FFF5F0); border-bottom:1px solid #FAF0E8; display:flex; align-items:center; gap:10px; flex-wrap:wrap;">
              <div style="width:30px; height:30px; background:var(--color-terracotta); border-radius:9px; display:flex; align-items:center; justify-content:center; flex-shrink:0;">
                <svg viewBox="0 0 24 24" width="14" height="14" fill="none" stroke="#fff" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="M18 8A6 6 0 0 0 6 8c0 7-3 9-3 9h18s-3-2-3-9"/><path d="M13.73 21a2 2 0 0 1-3.46 0"/></svg>
              </div>
              <div style="flex:1; min-width:0;">
                <div style="font-size:13px; font-weight:800; color:#3E352F; line-height:1.2;">Ringkasan Notifikasi & Pengingat</div>
                <div style="font-size:10.5px; color:var(--text-muted); margin-top:1px;">Hari ini dari semua modul aktifmu</div>
              </div>
              <!-- Stat chips -->
              <div style="display:flex; gap:6px; flex-wrap:wrap;">
                <div style="background:#FAF0EC; border:1px solid #F3D8CC; border-radius:7px; padding:3px 9px; text-align:center;">
                  <div style="font-size:13px; font-weight:800; color:var(--color-terracotta); line-height:1;">{{ allModuleItems.today.length }}</div>
                  <div style="font-size:9px; color:var(--text-muted); font-weight:600;">hari ini</div>
                </div>
                <div v-if="allModuleItems.reminders.length > 0" style="background:#EFF6FF; border:1px solid #BFDBFE; border-radius:7px; padding:3px 9px; text-align:center;">
                  <div style="font-size:13px; font-weight:800; color:#2563eb; line-height:1;">{{ allModuleItems.reminders.length }}</div>
                  <div style="font-size:9px; color:var(--text-muted); font-weight:600;">mendatang</div>
                </div>
                <div v-if="allModuleItems.missed.length > 0" style="background:#FEF2F2; border:1px solid #FECACA; border-radius:7px; padding:3px 9px; text-align:center;">
                  <div style="font-size:13px; font-weight:800; color:#ef4444; line-height:1;">{{ allModuleItems.missed.length }}</div>
                  <div style="font-size:9px; color:var(--text-muted); font-weight:600;">terlewat</div>
                </div>
              </div>
            </div>

            <!-- SECTION: Hari Ini -->
            <div v-if="allModuleItems.today.length > 0" style="padding:10px 14px; border-bottom:1px solid #FAF0E8;">
              <div style="font-size:10px; font-weight:700; color:var(--text-muted); letter-spacing:0.6px; text-transform:uppercase; margin-bottom:8px; display:flex; align-items:center; gap:5px;">
                <span style="width:6px; height:6px; background:var(--color-terracotta); border-radius:50%; display:inline-block;"></span>
                Hari Ini
              </div>
              <div style="display:flex; flex-direction:column; gap:6px;">
                <div v-for="item in allModuleItems.today" :key="item.id"
                     style="display:flex; align-items:center; gap:9px; padding:7px 10px; border-radius:9px; border:1px solid;"
                     :style="{ background: item.color + '0D', borderColor: item.color + '30' }">
                  <span style="font-size:13px; flex-shrink:0;">{{ item.icon }}</span>
                  <div style="flex:1; min-width:0;">
                    <div style="font-size:12px; font-weight:700; color:#3E352F; overflow:hidden; text-overflow:ellipsis; white-space:nowrap;">{{ item.title }}</div>
                    <div style="font-size:10px; color:var(--text-muted); font-weight:600;">{{ item.source }}{{ item.subtitle ? ' · ' + item.subtitle : '' }}</div>
                  </div>
                  <div v-if="item.time" :style="{ background: item.color + '15', color: item.color }"
                       style="flex-shrink:0; font-size:10.5px; font-weight:800; padding:2px 8px; border-radius:20px; white-space:nowrap;">
                    {{ item.time }}
                  </div>
                </div>
              </div>
            ...
            <!-- SECTION: Pengingat Mendatang -->
            <div v-if="allModuleItems.reminders.length > 0" style="padding:10px 14px; border-bottom:1px solid #FAF0E8;">
              <div style="font-size:10px; font-weight:700; color:var(--text-muted); letter-spacing:0.6px; text-transform:uppercase; margin-bottom:8px; display:flex; align-items:center; gap:5px;">
                <span style="width:6px; height:6px; background:#6366f1; border-radius:50%; display:inline-block;"></span>
                Pengingat Mendatang
              </div>
              <div style="display:flex; flex-direction:column; gap:5px;">
                <div v-for="item in allModuleItems.reminders" :key="item.id"
                     style="display:flex; align-items:center; gap:9px; padding:6px 10px; border-radius:9px; background:#F5F3FF; border:1px solid #DDD6FE;">
                  <span style="font-size:12px; flex-shrink:0;">⏰</span>
                  <div style="flex:1; min-width:0;">
                    <div style="font-size:12px; font-weight:700; color:#3E352F; overflow:hidden; text-overflow:ellipsis; white-space:nowrap;">{{ item.title }}</div>
                    <div style="font-size:10px; color:#6366f1; font-weight:600;">{{ item.date }} {{ item.time ? '· ' + item.time : '' }}</div>
                  </div>
                </div>
              </div>
            </div>
            ...
            <!-- SECTION: Terlewat -->
            <div v-if="allModuleItems.missed.length > 0" style="padding:10px 14px;">
              <div style="font-size:10px; font-weight:700; color:var(--text-muted); letter-spacing:0.6px; text-transform:uppercase; margin-bottom:8px; display:flex; align-items:center; gap:5px;">
                <span style="width:6px; height:6px; background:#ef4444; border-radius:50%; display:inline-block;"></span>
                Terlewat
              </div>
              <div style="display:flex; flex-direction:column; gap:5px;">
                <div v-for="item in allModuleItems.missed" :key="item.id"
                     style="display:flex; align-items:center; gap:9px; padding:6px 10px; border-radius:9px; background:#FEF2F2; border:1px solid #FECACA;">
                  <span style="font-size:12px; flex-shrink:0;">⚠️</span>
                  <div style="flex:1; min-width:0;">
                    <div style="font-size:12px; font-weight:700; color:#3E352F; overflow:hidden; text-overflow:ellipsis; white-space:nowrap;">{{ item.title }}</div>
                    <div style="font-size:10px; color:#ef4444; font-weight:600;">{{ item.date }} {{ item.time ? '· ' + item.time : '' }}</div>
                  </div>
                </div>
              </div>
            </div>
            ...
            <!-- Empty state panel -->
            <div v-if="allModuleItems.today.length === 0 && allModuleItems.reminders.length === 0 && allModuleItems.missed.length === 0"
                 style="padding:24px; text-align:center; color:var(--text-muted);">
              <div style="font-size:22px; margin-bottom:6px;">✅</div>
              <div style="font-size:12.5px; font-weight:700; color:#8F8175;">Semua bersih hari ini!</div>
              <div style="font-size:11px; margin-top:2px;">Tidak ada notifikasi atau pengingat aktif.</div>
            </div>
          </div>
          ...
          <div style="display:flex; align-items:center; gap:10px; margin-bottom:14px;">
            ...
            <span style="font-size:11px; font-weight:700; color:var(--text-muted); letter-spacing:0.5px; white-space:nowrap;">ACARA LOKAL</span>
            <div style="flex:1; height:1px; background:var(--color-sand);"></div>
          </div>
  ...
  computed: {
    ...
    // ── Rangkuman semua modul (Notif + Pengingat + Terlewat) untuk ditampilkan di Agenda ──
    allModuleItems() {
      const todayStr = new Date().toISOString().split('T')[0];
      const items = { today: [], reminders: [], missed: [] };

      // 1. Pengingat Hari Ini dari ws_habit_notifs
      try {
        const raw = WorkspaceStorage.getItem('ws_habit_notifs');
        if (raw) {
          JSON.parse(raw).forEach(h => {
            items.today.push({ id: h.id, title: h.title, subtitle: h.subtitle || 'Habit harian', time: h.time || '', icon: '🌿', color: h.color || '#10b981', source: 'Habit' });
          });
        }
      } catch(e) {}

      // 2. Pengingat manual hari ini (dari ws_manual_notifs - hanya today)
      try {
        const raw = WorkspaceStorage.getItem('ws_manual_notifs');
        if (raw) {
          JSON.parse(raw).filter(m => m.date === todayStr).forEach(m => {
            items.today.push({ id: m.id, title: m.title, subtitle: m.subtitle || 'Pengingat manual', time: m.time || '', icon: '⏰', color: '#6366f1', source: 'Manual', isManual: true });
          });
        }
      } catch(e) {}

      // 3. Job Plans hari ini
      try {
        const raw = WorkspaceStorage.getItem('personal_workspace_job_plans');
        if (raw) {
          JSON.parse(raw).filter(p => p.date === todayStr).forEach(p => {
            items.today.push({ id: 'plan-'+p.id, title: p.tasks || 'Job Plan', subtitle: p.category || 'Job Logbook', time: '', icon: '📋', color: '#f59e0b', source: 'Job Logbook' });
          });
        }
      } catch(e) {}

      // 4. Content Plan hari ini (dueDate)
      try {
        const raw = WorkspaceStorage.getItem('personal_workspace_content_items');
        if (raw) {
          JSON.parse(raw).filter(i => i.dueDate === todayStr).forEach(i => {
            items.today.push({ id: 'content-'+i.id, title: i.title, subtitle: i.platform || 'Content Plan', time: '', icon: '🎬', color: '#8b5cf6', source: 'Content' });
          });
        }
      } catch(e) {}

      // 5. Pengingat manual mendatang (bukan hari ini)
      try {
        const raw = WorkspaceStorage.getItem('ws_manual_notifs');
        if (raw) {
          JSON.parse(raw).filter(m => m.date > todayStr).sort((a,b) => a.date.localeCompare(b.date)).forEach(m => {
            items.reminders.push({ id: m.id, title: m.title, subtitle: m.subtitle || '', time: m.time || '', date: m.date, icon: '⏰', color: '#6366f1', isManual: true });
          });
        }
      } catch(e) {}

      // 6. Terlewat (ws_missed_tasks)
      try {
        ...
        if (raw) {
          ...
          log.slice(0, 5).forEach(entry => {
            (entry.tasks || []).forEach(t => {
              items.missed.push({ id: entry.date + '_' + t.id, title: t.title, time: t.time || '', date: entry.date, icon: '⚠️', color: '#ef4444' });
            });
          });
        }
      } catch(e) {}
      ...
      // Sort today by time
      items.today.sort((a,b) => (a.time || '99:99').localeCompare(b.time || '99:99'));
